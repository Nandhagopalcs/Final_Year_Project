...
# db = client['covid_19']
connection="mongodb+srv://nandhu17:nandhu@cluster0.q49fb.mongodb.net/covid?retryWrites=true&w=majority"
...
# past = {"_id":date,"day":"2020-12-19","name":"sabah","state":"kerala"}
# current=db['current']
...
# # p={"_id":"password","password":"password","role":"super_admin"}
# # admin_login.insert_one(p)
# p={"_id":0,"num":0}
# now=db['count']
# now.insert_one(p)
...
def login():
    if request.method == 'POST':
        credentials = request.form.to_dict()
        #checking
        if(credentials["user_email"]==""  or credentials["user_password"]==""):
            return render_template('index.html')
        login=db['login']
        existing_user = login.find_one(({"_id": credentials["user_email"]}))
        #
...
def processing():
    ...
    if(pred==0):
        ...
        past = {"_id":date,"day":day,"name":session['user_name'],"state":existing_user['state']}
        current=db['current']
        complete=db['complete']
        current.insert_one(past)
        complete.insert_one(past)
...
def record():
    if request.method =="POST":
        ...
        "user_diarrhoea":user_diarrhoea,"sore_throat":sore_throat,"difficulty_breathing":difficulty_breathing,"dizziness_confusion":dizziness_confusion,
        "headache":headache,"running_blocked_nose":running_blocked_nose,"loss_taste":loss_taste,"muscle_pain":muscle_pain,
        "fatique":fatique,"breath_shallow":"-1","breath_deep":"-1","cough_shallow":"-1","cough_heavy":"-1","vowel_a":"-1","vowel_e":"-1","vowel_o":"-1","counting_normal":"-1","counting_fast":"-1"}
        check.insert_one(past)
        session['cont_time']=dt_string
...
# @app.route('/meta')
# def meta():
#     return render_template("contribution_")
